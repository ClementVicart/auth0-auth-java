...
public class Auth0AutoConfiguration {
   ...
   *
   * <p>Builds the authentication options configuration.
   *
   * @param properties the Auth0 configuration properties from application configuration
   * @param domainResolverProvider optional {@link Auth0DomainResolver} bean for dynamic MCD
   *     resolution. When present, it takes precedence over static YAML config.
  ...
  public AuthOptions authOptions(
      Auth0Properties properties, ObjectProvider<Auth0DomainResolver> domainResolverProvider) {

    Auth0DomainResolver auth0DomainResolver = domainResolverProvider.getIfAvailable();

    AuthOptions.Builder builder = new AuthOptions.Builder().audience(properties.getAudience());

    if (auth0DomainResolver != null) {
      // Bridge Spring Boot's Auth0DomainResolver → core DomainResolver
      DomainResolver coreDomainResolver =
          coreContext -> {
            Auth0RequestContext springContext =
                new Auth0RequestContext(
                    coreContext.getUrl(), coreContext.getHeaders(), coreContext.getTokenIssuer());
            return auth0DomainResolver.resolveDomains(springContext);
          };
      builder.domainsResolver(coreDomainResolver);
   ...
   *
   * <p>Serves as the main entry point for verifying HTTP requests containing access tokens.
   *
