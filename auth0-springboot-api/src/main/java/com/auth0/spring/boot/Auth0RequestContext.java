...
/**
 * Immutable request context passed to {@link Auth0DomainResolver} for dynamic domain resolution in
 * Multi-Custom Domain (MCD) scenarios.
 *
 * <p>Contains all the information a resolver needs to determine which issuer domains are valid for
 * the incoming request:
 *
 * <ul>
 *   <li>{@code url} — the URL the API request was made to
 *   <li>{@code headers} — relevant HTTP request headers (lowercase keys)
 *   <li>{@code tokenIssuer} — the <b>unverified</b> {@code iss} claim from the JWT
 * </ul>
 *
 * <p><b>Warning:</b> The {@code tokenIssuer} has NOT been verified yet. It is provided as a routing
 * hint only and must not be trusted on its own.
 *
...
public final class Auth0RequestContext {
   ...
   *
   * <p><b>Warning:</b> This value has NOT been verified. Use it only as a routing hint (e.g., to
